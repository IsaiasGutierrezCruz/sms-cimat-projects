...
```{r data-loading, fig.width=10, fig.height=6, echo=FALSE}
...
- Dimensiones del dataset: `r dim(rendimiento)`
- Número de observaciones: `r nrow(rendimiento)`
- Número de variables: `r ncol(rendimiento)`


```{r data-exploration, fig.width=10, fig.height=6, echo=FALSE}
...
```{r toyota-class-analysis, fig.width=6, fig.height=4, echo=FALSE}
...
p1 <- ggplot(toyota_class_freq, aes(x = reorder(Class, n), y = n)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, max(toyota_class_freq$n) * 1.2))
...
```{r mazda-transmission-analysis, fig.width=6, fig.height=4, echo=FALSE}
...
p2 <- ggplot(mazda_transmission_freq, aes(x = reorder(Transmission, n), y = n)) +
  ...
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, max(mazda_transmission_freq$n) * 1.2))
...
```{r porsche-co2-analysis, fig.width=6, fig.height=4, echo=FALSE}
...
```{r dodge-consumption-analysis, fig.width=6, fig.height=4, echo=FALSE}
...
```{r best-vehicle-analysis, fig.width=6, fig.height=4, echo=FALSE}
...
top_vehicles %>%
  select(Ranking, Marca, Make_Model, Consumption_Combined, CO2_Emissions, Cost_Per_Year, Combined_Score) %>%
  kable(
    caption = "Top 10 Mejores Vehículos según Análisis Integral",
    col.names = c("Rank", "Marca", "Modelo",  "Consumo (L/100km)", 
                  "CO2 (g/km)", "Costo/Año ($)", "Score Total"),
  ) %>%
...
A continuación se muestra una comparación gráfica de los 5 mejores vehículos según el segmentado de acuerdo a los criterios de evaluación:

```{r best-vehicle-comparison, fig.width=6, fig.height=4, echo=FALSE}
...
Repositorio de Github: [Código](https://github.com/IsaiasGutierrezCruz/sms-cimat-projects/blob/main/r/scripts/statistics-in-research/rendimiento_automotriz.Rmd)